...
final class StickyCommandTest: XCTestCase {
    ...
    func testStickyOnFloatsTheWindow() async {
        ...
        await parseCommand("sticky --window-id 1 on").cmdOrDie.run(.defaultEnv, .emptyStdin)
    ...
    func testStickyToggleAndOff() async {
        let workspace = Workspace.get(byName: name).apply {
        ...
        await parseCommand("sticky --window-id 1 toggle").cmdOrDie.run(.defaultEnv, .emptyStdin)
        assertEquals(window.isSticky, true)

        await parseCommand("sticky --window-id 1 toggle").cmdOrDie.run(.defaultEnv, .emptyStdin)
        assertEquals(window.isSticky, false)

        await parseCommand("sticky --window-id 1 off").cmdOrDie.run(.defaultEnv, .emptyStdin)
        assertEquals(window.isSticky, false)
    }

    func testStickyWindowFollowsWorkspaceSwitch() async {
